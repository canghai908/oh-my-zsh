...
var Manage = {
    ...
    ActivityList: function (pageIndex, StateStr, ajax_url) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                ' </ul>' +
                '</div>';
                        if (list[i].state == "0") {          //如果是中
                           
                        }
                        else if (list[i].state == "1") {          //如果是审核中
                            // Manage.htmlDiv +=
                            //    '<ul class="operation mt15 mr15 fr">' +
                            //   '<li class="application"><a Onclick="Manage.ApplyReturn(' + ManageViews[i].ActivityId + ',' + ManageViews[i].UserId + ');">立即撤回</a></li>' +
                            //  '<li class="analysis"><a Onclick="Manage.EffectAnalysis(' + ManageViews[i].ActivityId + ')">效果分析</a> </li>' +
                            //      '<li class="promotion"><a  Onclick="Manage.GetLink(' + ManageViews[i].ActivityId + ',1);">获取推广链接</a></li>' +
                            //  '</ul>' + '</div>';
                        }
                        else if (list[i].state == "2" || list[i].state == "3") {
                        //     Manage.htmlDiv +=
                        //      '<ul class="operation mt15 mr15 fr">' +
                        //      '<li class="applicationtj"><a Onclick="Manage.Detailed(' + ManageViews[i].ActivityId + ','+list[i].merchant_id+');">编辑</a></li>' +
                        // '<li class="analysis"><a Onclick="Manage.EffectAnalysis(' + ManageViews[i].ActivityId + ')">效果分析</a></li>' +
                        // "<li class='promotion'><a  Onclick=\'Manage.GetLink(" + ManageViews[i].ActivityId + ",1)\'>获取推广链接</a></li>" +
                        // '</ul>' + '</div>';
                        }
                        else if (list[i].state == "4") {    //如果进行中已经结束
                            // Manage.htmlDiv +=
                        ...
                        else if (list[i].state == "5" || list[i].state == "6") {               //如果是暂存
                            // Manage.htmlDiv += '<ul class="operation  mt15 mr15 fr"><li class="applicationtj"><a Onclick="Manage.Detailed(' + ManageViews[i].ActivityId + ','+list[i].merchant_id+');">编辑</a></li><li class="promotion"><a Onclick="Manage.GetLink(' + ManageViews[i].ActivityId + ",'Save')\">获取二维码</a></li><li class='Deletet'><a Onclick=\"Manage.DeleteSave(" + ManageViews[i].ActivityId + ",'" + ManageViews[i].Title + "');\">删除</a></li></ul></div>"
                            // ;
                        }
                    }
    ...
    Detailed: function (merchant_id, id) {
        ...
        location.href = "/active/edit.html";           //跳转编辑活动页面
    },
    /// <summary>
    /// 删除暂存记录
    /// </summary>
    ...
    DeleteSave: function (ActivitieId, title) {
        ...
        $("#BtnOk").click(function () {                    //删除对话框关闭
            ...
            $.ajax({
                url: "./data/Activity/ActivitiesDeleteSave/success.json",
                type: 'GET',
                async: false,
                contentType: "application/x-www-form-urlencoded",
                data: { "ActivitieId": ActivitieId },
                evalJSON: false,
                evalJS: false,
                success: function (data) {
                    if (data.result == "true") {
