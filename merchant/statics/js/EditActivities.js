...
var editactivitie = {
    ...
    tijiao: function() {
        ...
        B.title = editactivitie.converthtml($.trim($("#activityTitle").val()))|| "默认标题";
        B.description = "";
		...
		B.start_datetime = startDate2 + "T" + $("#startHour").val() + ":00";
		var k1 = $("#endHour").val();
		if( k1 == "0:00:"){
			k1 = "00:00";
		};
		B.end_datetime = startDate4 + "T" + k1 + ":00";
        B.SpreadLink = "";
		B.share_cycle = parseInt($("#pri_zhouqi1").val()) || 24;
		B.prize_policy_array = [];
        for (var A = 1; A <= awardsCount; A++) {
			var C = {
				ID:A,
				condition_min: parseInt($.trim($("#pri_ShareCount" + A).val()) == "" ? "0" : $.trim($("#pri_ShareCount" + A).val())),
				prize: editactivitie.converthtml($.trim($("#pri_Content" + A).val())),
				IsLimitPrizeCount: true,
				total_number: parseInt($.trim($("#pri_Count" + A).val()) == "" ? "0" : $.trim($("#pri_Count" + A).val()))
			};
        ...
        $.ajax({
            url: "/merchant/promotion_edit_submit?promotion_id=" + activitis.id,
            type: "post",
    ...
    zancun: function() {
        ...
        var C = {};
		C.title = editactivitie.converthtml($.trim($("#activityTitle").val())) || "默认标题";
		C.description = "";
		...
		C.end_datetime = startDate4 + "T" + k1 + ":00";

		C.SpreadLink = "";
		C.share_cycle = parseInt($("#pri_zhouqi1").val()) || 24;
        C.prize_policy_array = [];
        for (var B = 1; B <= awardsCount; B++) {
			var D = {
				id:B,
				condition_min: parseInt($.trim($("#pri_ShareCount" + B).val()) == "" ? "0" : $.trim($("#pri_ShareCount" + B).val())),
				prize: editactivitie.converthtml($.trim($("#pri_Content" + B).val())),
				IsLimitPrizeCount: true,
				total_number: parseInt($.trim($("#pri_Count" + B).val()) == "" ? "0" : $.trim($("#pri_Count" + B).val()))
			};
        ...
        C.Phone = A;
        $.ajax({
            url: "/merchant/promotion_edit_save?promotion_id=" + activitis.id,
            type: "post",
			...
			success: function(F) {
				if (F.err_msg) {
                    ...
                    //location.href = "/merchant/activity_Save.html";
                };
    ...
    saveInit: function(D,token) {
        ...
        $("#tijiaoOKDialog").dialog({
            ...
            close: function() {
                ...
                location.href = "/merchant/activity_Checking.html";
            }
        });
